...
public class ObjectUtils {
	...
	public static boolean isNull(Object obj){
		if(isString(obj)) return  StringUtil.isNull((String) obj);
	...
	public static <T> T copyOf(T obj){
		if(isNull(obj)) throw new NullPointerException("obj is null");
		else if(isString(obj)) return (T) copyOf((String)obj);
		else if(isNumber(obj)) return (T) copyOf((Number)obj);
		else if(isList(obj)) return (T) copyOf((List<?>)obj);
		else if(isMap(obj)) return (T) MapUtil.copyOf((Map<?, ?>) obj);
		else return beanCopy(obj);
	}
